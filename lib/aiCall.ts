...
async function chatJSON<T>(opts: {
  ...
  for (let attempt = 1; attempt <= 4; attempt++) {
    try {
      ...
      if (usage) {
        // Rough cost hints (tune later if needed)
        const perMTok = 0.000000; // keep 0 during testing to avoid misleading math
        const dollars = (usage.total_tokens / 1_000_000) * perMTok;
        console.log(`[AI:${opts.task}] model=${model} tokens=${usage.total_tokens} est_cost=$${dollars.toFixed(6)}`);
      } else {
        console.log(`[AI:${opts.task}] model=${model} (no usage returned)`);
      }
